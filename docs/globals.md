[**shadowdark-parser**](README.md)

***

# shadowdark-parser

## Type Aliases

- [Alignment](type-aliases/Alignment.md)
- [Attack](type-aliases/Attack.md)
- [Entity](type-aliases/Entity.md)
- [MagicItem](type-aliases/MagicItem.md)
- [Monster](type-aliases/Monster.md)
- [Spell](type-aliases/Spell.md)
- [Table](type-aliases/Table.md)
- [Trait](type-aliases/Trait.md)

## Variables

- [allCapsStrategy](variables/allCapsStrategy.md)

## Functions

- [bulkParse](functions/bulkParse.md)
- [identify](functions/identify.md)
- [parse](functions/parse.md)
- [parseAttack](functions/parseAttack.md)
- [parseMagicItem](functions/parseMagicItem.md)
- [parseRollTable](functions/parseRollTable.md)
- [parseSpell](functions/parseSpell.md)
- [parseStatblock](functions/parseStatblock.md)
- [parseTraits](functions/parseTraits.md)
